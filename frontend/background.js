...
chrome.runtime.onMessage.addListener(function(request, sender, sendResponse) {
    if (request.action == 'updateBadge') {
        console.log('Updating badge for tab', sender.tab.id);
        chrome.action.setBadgeText({text: request.data.political_rating.toString(), tabId: sender.tab.id});
        chrome.action.setBadgeBackgroundColor({color: [0, 255, 0, 255], tabId: sender.tab.id});
    }
});